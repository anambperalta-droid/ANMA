...
const ACTION_MAP = {
  ...
  /* ──────────────────────────────────────────────────────────────
     🧪 INSUMO — Redirige a insumos para reponer materia prima.
     Extrae: route → /insumos
  ────────────────────────────────────────────────────────────── */
  insumo: {
    label: 'Reponer Insumo',
    icon: 'fa-flask',
    color: '#D97706',
    bg: '#FEF3C7',
    handler: (alert, { nav, setOpen }) => {
      setOpen(false)
      nav(alert.route || '/insumos')
    },
  },
...
function buildAlerts(budgets, products, insumos) {
  const alerts = []
  ...
  // ── INSUMOS ALERTS ──
  ;(insumos || []).forEach(ins => {
    const stock = Number(ins.stock ?? -1)
    const minStock = Number(ins.minStock ?? 0)
    if (!ins.name || stock < 0) return
    const iid = typeof ins.id === 'number' ? ins.id : 0
    if (stock === 0 && minStock >= 0) {
      alerts.push({
        id: `insumo0-${ins.id}`,
        level: 'critical',
        category: 'insumo',
        icon: 'fa-cubes',
        title: `Sin stock insumo: ${ins.name}`,
        body: `Se agotó — puede frenar la producción`,
        route: '/insumos',
        ts: iid,
      })
    } else if (minStock > 0 && stock <= minStock) {
      alerts.push({
        ...
        category: 'insumo',
        icon: 'fa-cubes',
        title: `Insumo bajo: ${stock} ${ins.name}`,
        body: `Mínimo: ${minStock} — reponé para no frenar producción`,
        route: '/insumos',
        ts: iid,
      })
    }
...
/* Color del ÍCONO por categoría (independiente del semáforo del borde).
   Regla: el borde-izquierdo dice "¿qué tan urgente?"; el ícono dice "¿qué tipo?".
   Así Ana ve de un vistazo dónde mirar y qué tocar. */
...
function groupAlerts(alerts) {
  // Agrupa alertas consecutivas por categoría + acción (misma pinta y CTA).
  const out = []
  let bucket = null
...
export default function NotificationBell({ extraCount = 0, className = '', variant = 'topbar' }) {
  ...
  // desde el TaskFab).
  // variant='sidebar' → estilos adaptados al quick-bar del sidebar (fondo neutro,
  // ícono con color de marca cuando hay pendientes). 'topbar' = comportamiento original.
  const { get } = useData()
  ...
  // Grupos expandidos (por key "categoria:label"). Un grupo con ≥3 alertas iguales
  // se muestra colapsado por defecto (evita la "pared roja" de entregas vencidas).
  const [expandedGroups, setExpandedGroups] = useState(() => new Set())

  const budgets  = get('budgets')
  const products = get('products')
  const insumos  = get('insumos')
  const localAlerts = useMemo(() => buildAlerts(budgets, products, insumos), [budgets, products, insumos])

  /* ── Admin alerts (Supabase, solo si isGlobalAdmin) ─────────────
     Reemplaza el useRealtimeSignups previo que solo funcionaba en /admin.
     Ahora vive en Topbar → funciona en toda la app. */
  const adminHook = useAdminAlerts(isGlobalAdmin)
  const adminAlerts = useMemo(() => (adminHook.alerts || []).map(a => ({
    id:        `admin-${a.id}`,
    _adminId:  a.id,                 // id real en la tabla
    _isAdmin:  true,
    _readAt:   a.read_at,
  ...
  // Toast + browser notification cuando llega un admin alert nuevo por realtime
  ...
  const allAlerts = useMemo(() => {
    // Admin alerts primero (más críticas — pueden ser signup nuevo, error, etc.)
    // Luego alertas locales por su propio ordenamiento.
    return [...adminAlerts, ...localAlerts]
  }, [adminAlerts, localAlerts])
  ...
  // Read state: para admin alerts viene de la tabla (a._readAt), para locales de readIds set
  ...
  const markRead = useCallback((alertOrId) => {
    // Puede recibir el objeto alert completo o solo el id (compat legacy)
  ...
  const markAllRead = useCallback(() => {
    // Locales via localStorage + Supabase batch
    ...
    // Admin via hook (update en tabla)
