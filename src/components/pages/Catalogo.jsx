...
export default function Catalogo() {
  ...
  const handleCostChange = (val) => {
    setF('cost', val)
    // Auto-calculate prices if empty or were auto-generated
  ...
  const getInsumoName = (id) => { const ins = insumosList.find(x => x.id === id); return ins ? ins.name : '—' }
  ...
  }, [moveModal, csvModal, bulkModal, modal])
  ...
  return (
    <div className="page active" style={{ animation: 'pgIn .25s ease both' }}>
      ...
      <div className="tbl-card">
        <table>
          <thead><tr>
            <th>Producto</th><th>Categoría</th><th>Proveedor</th><th style={{ textAlign: 'right' }}>Costo</th>
            <th style={{ textAlign: 'right' }}>P. Público</th><th style={{ textAlign: 'right' }}>P. Mayorista</th>
            <th style={{ textAlign: 'center' }}>Stock</th><th>Acciones</th>
          </tr></thead>
          <tbody>
            {loading ? [1,2,3,4].map(i => (
              <tr key={i}><td colSpan={8}><div className="sk sk-text" style={{ height: 18, width: `${50 + Math.random() * 40}%` }} /></td></tr>
            ...
            }) : <tr><td colSpan={8}><div className="empty"><div className="ico"><i className="fa fa-box-open" /></div><p>Sin productos</p></div></td></tr>}
          </tbody>
        </table>
      </div>
          ...
          <div className="modal" style={{ maxWidth: 740 }}>
            ...
            <div style={{ background: 'var(--surface2)', borderRadius: 10, padding: '14px 16px', marginTop: 8, border: '1.5px solid var(--border)' }}>
              ...
              {(form.insumos || []).length === 0 && (
                <div style={{ fontSize: 11, color: 'var(--txt3)', textAlign: 'center', padding: '8px 0' }}>Sin insumos asociados (opcional)</div>
              )}
