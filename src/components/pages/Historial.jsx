...
/* ── Modal de registro de pagos individuales ──────────────────────
   Maneja un array de pagos {date, amount, method, notes} por presupuesto.
   El payStatus se recalcula automáticamente desde la suma vs totalFinal.   */
const PAY_METHODS = [
  ...
  return (
    <div className="modal-bg open" onClick={e => { if (e.target === e.currentTarget) onClose() }}>
      <div className="modal" style={{ maxWidth: 560, padding: '18px 22px 18px', maxHeight: 'none', height: 'auto', overflow: 'visible' }} onClick={e => e.stopPropagation()}>
        ...
        {/* Warning si paid sin payments — chip inline */}
        {inconsistentPaid && (
        ...
        {/* Lista de pagos — compacta, máx 3 filas visibles, después scroll interno */}
        {payments.length > 0 && (
        ...
        <div style={{ background: 'rgba(124,58,237,.04)', border: '1px solid rgba(124,58,237,.15)', borderRadius: 10, padding: '11px 13px' }}>
          ...
          {/* Status del monto — 1 línea inline */}
...
function LossReasonModal({ budget, onSave, onClose }) {
  ...
  // Si hay pagos registrados, preguntar qué pasó con esa plata.
  // null = aún no contestó. true = me quedé. false = devolví.
  const hasPayments = budget && (Array.isArray(budget.payments) && budget.payments.length > 0)
                                 || (budget && (budget.payStatus === 'partial' || budget.payStatus === 'paid'))
  const paidAmount = (() => {
    if (!budget) return 0
  ...
  return (
    <div className="modal-bg open" onClick={e => { if (e.target === e.currentTarget) onClose() }}>
      <div className="modal" style={{ maxWidth: 460 }}>
        ...
        {/* Si hay pagos → preguntar qué pasó con esa plata */}
        {hasPayments && (
...
function BarChart({ data, prevData = [], type = 'income' }) {
  ...
  // Smart X axis: when daily view has too many empty days, keep only days with activity (current or previous)
...
/**
 * Cuánto se cobró el día/fecha dado. Suma los `payments[]` que coincidan con esa
 * fecha exacta. Si el budget NO tiene payments[] (legacy) cae al criterio
 * histórico: cobrado total atribuido a la fecha de creación del presupuesto.
...
export default function Historial() {
  ...
  // ── Period filter (memoized — won't recalculate on search/sort changes) ──
  ...
  // ── Sparklines: últimos 14 días (memoized — only recalculates when data changes) ──
  const { sparkBrutas, sparkCaja, sparkTicket } = useMemo(() => {
    const brutas = []; const caja = []; const ticket = []
    for (let i = 13; i >= 0; i--) {
      const d = new Date(); d.setDate(d.getDate() - i); d.setHours(0,0,0,0)
      const ds = d.toISOString().slice(0, 10)
      // Ventas brutas del día: presupuestos creados ese día (excluye perdidos)
      const dayBs = budgets.filter(b => b.date === ds && b.status !== 'lost')
      brutas.push(dayBs.reduce((s, b) => s + (b.total || 0), 0))
      // Caja del día: pagos efectivamente recibidos. Excluye perdidos sin seña preservada.
  ...
  // ── MODO HOY (memoized — only recalculates when budgets change) ──
  ...
  // ── Chart data (memoized — only recalculates when budgets or period changes) ──
  ...
  const { chartData, prevChartData, gainData } = useMemo(() => {
    ...
    if (period === 'thismonth') {
      ...
      // Cash flow REAL: sumamos los payments[] del día (no la fecha de creación del presupuesto)
  ...
  // ── Filtered + sorted budget list (memoized — recalculates only on list-relevant changes) ──
  ...
  const confirmLoss = ({ reason, keptDeposit }) => {
    ...
    if (b) {
      // Determinar qué pasa con los pagos:
      //   keptDeposit=true  → preservamos payments[] (cuentan como ingreso)
      //   keptDeposit=false → limpiamos payments[] (la plata se devolvió)
      //   keptDeposit=null  → sin pagos registrados, no aplica
  ...
  const STATUS_COLORS = {
    ...
    delivered:   { bg: '#DCFCE7', color: '#15803D', border: '#4ADE80' },
  ...
  // Si detectamos uno de esos prefijos, anteponemos una comilla simple para neutralizar el riesgo.
  const csvEsc = (v) => {
    let s = String(v ?? '')
    if (/^[=+\-@\t\r]/.test(s)) s = "'" + s
  ...
  const bulkExportCSV = () => {
    ...
    downloadCSV([CSV_HEADERS.join(','), ...sel.map(budgetToRow)], `pedidos_sel_${new Date().toISOString().slice(0,10)}.csv`)
  }
  ...
  const exportCSV = () => {
    downloadCSV([CSV_HEADERS.join(','), ...budgets.map(budgetToRow)], `pedidos_${new Date().toISOString().slice(0,10)}.csv`)
  }
  ...
  const insights = useMemo(() => {
    ...
    if (cobrosVencidos.length >= 3) {
      out.push({
        ...
        value: money(cobrosVencidosMonto),
  ...
  return (
    <div className="page active" style={{ animation: 'pgIn .25s ease both' }}>
      ...
      {/* Mobile control bar: fila de acciones (período · exportar · nuevo) +
          fila de pestañas (4 columnas iguales, sin scroll horizontal). Patrón
          Stripe/Notion mobile: si son ≤5 tabs, todas visibles → mejor UX que
          scroll oculto. */}
            ...
            <div style={{ marginBottom: 22 }}>
              <div className="hoy-importa-header">
                ...
                <button onClick={toggleTodayCollapsed}
                  ...
                  style={{ marginLeft: 'auto', background: 'transparent', border: '1px solid var(--border)', borderRadius: 9, padding: '6px 12px', fontSize: 11.5, fontWeight: 600, color: 'var(--txt3)', cursor: 'pointer', fontFamily: 'inherit', display: 'inline-flex', alignItems: 'center', gap: 6, transition: 'all .15s' }}>
                ...
                <div onClick={() => { setQuickFilter('sin_cobrar'); setTab('lista') }}
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ fontSize: 18, fontWeight: 700, color: 'var(--txt)', lineHeight: 1.1, fontFamily: "'Space Grotesk','Inter',sans-serif" }}>{cobrosVencidos.length} <span style={{ fontSize: 11, color: 'var(--txt3)', fontWeight: 500 }}>{cobrosVencidos.length === 1 ? 'pedido' : 'pedidos'}</span></div>
                    </div>
                ...
                <div onClick={() => setTab('lista')}
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ fontSize: 18, fontWeight: 700, color: 'var(--txt)', lineHeight: 1.1, fontFamily: "'Space Grotesk','Inter',sans-serif" }}>{entregasHoy.length} <span style={{ fontSize: 11, color: 'var(--txt3)', fontWeight: 500 }}>{entregasHoy.length === 1 ? 'pedido' : 'pedidos'}</span></div>
                    </div>
                ...
                <div onClick={() => setTab('seguimiento')}
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ fontSize: 18, fontWeight: 700, color: 'var(--txt)', lineHeight: 1.1, fontFamily: "'Space Grotesk','Inter',sans-serif" }}>{aConfirmar.length} <span style={{ fontSize: 11, color: 'var(--txt3)', fontWeight: 500 }}>seguimientos</span></div>
                    </div>
        ...
        <>
                ...
                {filteredBudgets.length ? filteredBudgets.map(b => {
                  ...
                  return (
                    <tr key={b.id} className={selectedIds.has(b.id) ? 'selected' : ''} style={selectedIds.has(b.id) ? { background: 'var(--brand-xlt)' } : undefined}>
                      ...
                      <td data-cell="total" data-label="Total" style={{ textAlign: 'right', fontWeight: 700, color: 'var(--money)', fontVariantNumeric: 'tabular-nums', letterSpacing: '-.01em', fontSize: 13.5, fontFamily: "'Space Grotesk','Inter',sans-serif" }}>
                        <div>{money(b.total)}</div>
                        <div className="col-hide-mobile" style={{ fontSize: 10.5, fontWeight: 600, color: hidden ? 'var(--txt4)' : (b.totalGain == null ? 'var(--txt4)' : '#16A34A'), marginTop: 2, fontStyle: b.totalGain == null ? 'italic' : undefined }}>
                          {b.totalGain == null ? 'gan. pendiente' : `+${money(b.totalGain)} gan.`}
                        </div>
                      </td>
                      <td data-cell="estado" data-label="Estado" style={{ whiteSpace: 'nowrap' }}>
                        <div>
                          <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6 }}>
                            ...
                            <select
                              style={{ fontSize: 11, padding: '2px 2px 2px 0', border: 'none', background: 'transparent', color: 'var(--txt2)', cursor: 'pointer', outline: 'none', fontWeight: 600, fontFamily: 'inherit' }}
                              value={b.status}
                      ...
                      <td data-cell="pago" data-label="Pago" style={{ whiteSpace: 'nowrap', verticalAlign: 'middle' }}>
                        ...
                        {(() => {
                          ...
                          const statusBg = b.payStatus === 'paid' ? '#F0FDF4'
                                         ...
                                         : '#FEF2F2'
                          return (
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 4, minWidth: 130, maxWidth: 170 }}>
                              <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, background: statusBg, padding: '3px 9px', borderRadius: 99, alignSelf: 'flex-start', maxWidth: 'fit-content' }}>
                                <span style={{ width: 6, height: 6, borderRadius: '50%', background: statusColor, flexShrink: 0, display: 'inline-block' }} />
                                <select
                                  style={{ fontSize: 11.5, fontWeight: 700, padding: 0, border: 'none', background: 'transparent', color: statusColor, cursor: 'pointer', outline: 'none', fontFamily: 'inherit' }}
                              ...
                              {b.payStatus === 'paid' && hasPayments ? (
                                <span style={{ fontSize: 10.5, color: 'var(--txt4)' }}>
                                  <i className="fa fa-check" style={{ color: '#16A34A', marginRight: 4 }} />
                                  {pays.length} pago{pays.length > 1 ? 's' : ''}
                                </span>
                              ) : b.payStatus === 'partial' ? (
                                <button onClick={e => { e.stopPropagation(); setPaymentsBudget(b) }}
                                  title="Ver / agregar pagos"
                                  style={{ background: 'transparent', border: 'none', color: 'var(--brand)', cursor: 'pointer', padding: 0, fontSize: 10.5, fontWeight: 700, textAlign: 'left', fontFamily: 'inherit' }}>
                                  {fmt(paid)}<span style={{ color: 'var(--txt4)', fontWeight: 400 }}> / {fmt(totalDue)}</span>
                                </button>
                              ) : b.payStatus === 'paid' ? (
                                <button onClick={e => { e.stopPropagation(); setPaymentsBudget(b) }}
                                  title="Registrar comprobante del pago"
                                  style={{ background: 'transparent', border: 'none', color: '#D97706', cursor: 'pointer', padding: 0, fontSize: 10.5, fontWeight: 600, textAlign: 'left', fontFamily: 'inherit' }}>
                                  <i className="fa fa-triangle-exclamation" style={{ marginRight: 4 }} />Sin comprobante
                                </button>
                              ) : totalDue > 0 && (
                                <button onClick={e => { e.stopPropagation(); setPaymentsBudget(b) }}
                                  title="Registrar cobro"
                                  style={{ background: 'var(--brand-xlt, #F5F3FF)', border: '1px solid var(--brand)', color: 'var(--brand)', cursor: 'pointer', padding: '3px 10px', fontSize: 10.5, fontWeight: 700, borderRadius: 6, alignSelf: 'flex-start', fontFamily: 'inherit', display: 'inline-flex', alignItems: 'center', gap: 4 }}>
                                  <i className="fa fa-hand-holding-dollar" /> Cobrar
                                </button>
                              )}
                            </div>
                          )
                  ...
                  <tr className="hist-empty-row"><td colSpan={8}><div className="empty"><div className="ico"><i className="fa fa-file-invoice" /></div><p>No hay presupuestos con este filtro</p></div></td></tr>
                )}
              </tbody>
            </table>
          </div>
          <div style={{ marginTop: 8, display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 6 }}>
            <span style={{ fontSize: 11, color: 'var(--txt3)' }}>
              ...
              {filter !== 'all' && <span style={{ marginLeft: 4, padding: '1px 7px', background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 10, fontWeight: 600, color: 'var(--txt3)' }}>{STATUS_MAP[filter]}</span>}
            </span>
          </div>
        </>
      )}
