...
function KpiCard({ label, value, delta, isKey, sparkData, sparkColor }) {
  ...
  return (
    <div className={`bento-kpi ${isKey ? 'bento-kpi-key' : ''}`} style={{ position: 'relative' }}>
      {/* Accent dot — sutil indicador de la métrica */}
                ...
                {filteredBudgets.length ? filteredBudgets.map(b => {
                  ...
                  return (
                    <tr key={b.id} className={selectedIds.has(b.id) ? 'selected' : ''} style={selectedIds.has(b.id) ? { background: 'var(--brand-xlt)' } : undefined}>
                      ...
                      <td className="col-hide-mobile" data-cell="gan" data-label="Ganancia" style={{ textAlign: 'right', color: hidden ? 'var(--txt4)' : (b.totalGain == null ? 'var(--txt4)' : '#16A34A'), fontWeight: 700, fontVariantNumeric: 'tabular-nums', fontFamily: 'ui-monospace,SFMono-Regular,monospace', fontSize: 12, fontStyle: b.totalGain == null ? 'italic' : undefined }}>{b.totalGain == null ? 'Pendiente' : money(b.totalGain)}</td>
