...
export default function Proveedores() {
  ...
  const [priceModal, setPriceModal] = useState(null) // { product } cuando está abierto
  ...
  const EMPTY_FORM = { name: '', contact: '', wa: '', rubro: '', email: '', notes: '', cuit: '', ivaCondition: '', paymentTerm: '', cbu: '', leadTime: '' }
  const openEdit = (s) => { setDetailSupplier(null); setForm(s ? { ...s } : { ...EMPTY_FORM }); setModal(true) }
  const save = () => {
  ...
  /* ── Concentración de compras: ¿qué % de productos viene del top proveedor? ── */
  ...
  const supplierScore = (s) => {
    ...
    // Histórico de precios estable: +10 si el promedio de subas es <10%, −10 si >25%
    ...
    // Actividad reciente: +10 si pedido en últimos 30 días
    const lastDays = supplierLastActivity(s)
    if (lastDays !== null && lastDays <= 30) { score += 10; factors.push('Activo recientemente +10') }
  ...
  const savePriceChange = () => {
    ...
    // Agregar entrada al histórico del proveedor
    const entry = {
    ...
    <div className="page active" style={{ animation: 'pgIn .2s ease both' }}>
      ...
      {(dismissed.reorder || dismissed.concentration) && (
        <button onClick={restoreAlerts}
          style={{ display: 'inline-flex', alignItems: 'center', gap: 5, padding: '4px 11px', borderRadius: 20, fontSize: 11, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit', border: '1.5px solid var(--border)', background: 'transparent', color: 'var(--txt3)', marginBottom: 10 }}
          title="Volver a mostrar alertas que ocultaste">
          <i className="fa fa-bell" style={{ fontSize: 10 }} /> Mostrar alertas ocultas
        </button>
      )}
      ...
      {totalLowStock > 0 && !dismissed.reorder && (
        <div style={{ background: 'rgba(220,38,38,.06)', border: '1px solid rgba(220,38,38,.25)', borderRadius: 10, marginBottom: 10, overflow: 'hidden' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px', fontSize: 12 }}>
            <i className="fa fa-bell" style={{ color: '#DC2626', fontSize: 14, cursor: 'pointer' }} onClick={() => setReorderOpen(v => !v)} />
            <div style={{ flex: 1, cursor: 'pointer' }} onClick={() => setReorderOpen(v => !v)}>
              ...
              <span style={{ color: 'var(--txt3)', marginLeft: 6 }}>
                · {allLowStockBySupplier.length} proveedor{allLowStockBySupplier.length !== 1 ? 'es' : ''} involucrado{allLowStockBySupplier.length !== 1 ? 's' : ''}
              </span>
            </div>
            <i className={`fa fa-chevron-${reorderOpen ? 'up' : 'down'}`} style={{ color: 'var(--txt3)', fontSize: 11, cursor: 'pointer' }} onClick={() => setReorderOpen(v => !v)} />
            <button onClick={() => dismissAlert('reorder')}
      ...
      <style>{`
        ...
        .prov-mob-acts{flex-shrink:0;display:flex;gap:2px;align-items:center}
        .prov-mob-act{position:relative;width:30px;height:30px;border-radius:50%;border:1.5px solid var(--border2);background:var(--surface2);color:var(--txt2);display:inline-flex;align-items:center;justify-content:center;cursor:pointer;font-size:12px;font-family:inherit;-webkit-tap-highlight-color:transparent;transition:opacity .12s,transform .12s;flex-shrink:0}
        .prov-mob-act::before{content:'';position:absolute;top:-7px;right:-7px;bottom:-7px;left:-7px}
        .prov-mob-act:active{opacity:.7;transform:scale(.9)}
        ...
        {loading ? [1,2,3,4].map(i => (
          ...
          return (
            <div key={s.id} className="prov-mob-card" onClick={() => openDetail(s)}>
              ...
              <div style={{ flexShrink:0,display:'flex',gap:1,alignItems:'center' }} onClick={e => e.stopPropagation()}>
        ...
        <div className="modal-bg open" style={{ zIndex: 700 }} onClick={e => { if (e.target === e.currentTarget) setModal(false) }}>
          <div className="modal-form-card" style={{ maxWidth: 620 }}
            ...
            <div style={{ flex: 1, overflowY: 'auto', minHeight: 0, padding: '18px 28px 4px', WebkitOverflowScrolling: 'touch' }}>
              ...
              <div className="fg"><label>Notas internas</label><textarea value={form.notes} onChange={e => setF('notes', e.target.value)} rows={3} placeholder="Condiciones especiales, recordatorios, acuerdos..." /></div>
            </div>
        ...
        <div className="modal-bg open" style={{ alignItems: 'flex-end', padding: 0 }} onClick={e => { if (e.target === e.currentTarget) { setImportModal(false); setCsvPreview([]) } }}>
          <div style={{ width: '100%', maxWidth: 640, background: 'var(--surface)', borderRadius: '20px 20px 0 0', display: 'flex', flexDirection: 'column', maxHeight: '92dvh', overflow: 'hidden', boxShadow: '0 -8px 40px rgba(0,0,0,.18)', animation: 'slideUp .25s cubic-bezier(.32,.72,0,1) both' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 20px 12px', flexShrink: 0 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                <div style={{ width: 38, height: 38, borderRadius: 12, background: 'var(--brand-xlt)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--brand)', fontSize: 17 }}><i className="fa fa-truck" /></div>
                <div>
                  <div style={{ fontSize: 16, fontWeight: 800, letterSpacing: '-.3px' }}>Importar proveedores</div>
                  <div style={{ fontSize: 11, color: 'var(--txt3)', marginTop: 1 }}>Planilla CSV · Excel · Google Sheets</div>
                </div>
              </div>
                ...
                <div>
                  ...
                  <div style={{ overflowX: 'auto', border: '1px solid var(--border)', borderRadius: 10, maxHeight: 340, overflowY: 'auto' }}>
                    <table style={{ fontSize: 11, minWidth: 480 }}>
                      ...
                      <tbody>
                        {csvPreview.slice(0, 20).map((s, i) => (<tr key={i}><td style={{ color: 'var(--txt4)', textAlign: 'center' }}>{i+1}</td><td><b>{s.name}</b></td><td>{s.contact}</td><td>{s.wa}</td><td>{s.rubro}</td><td>{s.email}</td></tr>))}
            ...
            <div className="mfooter" style={{ flexShrink: 0, borderTop: '1px solid var(--border)', padding: '12px 20px' }}>
              ...
              <button className="btn btn-primary" onClick={doImport} disabled={!csvPreview.length}><i className="fa fa-file-import" /> {csvPreview.length > 0 ? `Importar ${csvPreview.length} proveedores` : 'Importar'}</button>
            </div>
          </div>
        </div>
      )}
