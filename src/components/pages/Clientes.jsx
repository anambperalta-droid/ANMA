...
export default function Clientes() {
  ...
  const [form, setForm] = useState({ company: '', contact: '', wa: '', email: '', rubro: '', notes: '', discount: 0 })
  const [newNote, setNewNote] = useState('')
  const [previewBudget, setPreviewBudget] = useState(null)
  const fileRef = useRef(null)
  const [csvPreview, setCsvPreview] = useState([])
  const [isDragging, setIsDragging] = useState(false)
  const [importTab, setImportTab] = useState('archivo')  // 'archivo' | 'pegar'
  ...
  // Kebab menu de acciones (mobile card): 1 solo botón (3 puntos) → popover con
  // Recontactar / Editar / Eliminar. Evita apilar 3 iconos redondos en el cel.
  ...
  const save = async () => {
    ...
    // Validaciones formato (email / WhatsApp / CUIT) — solo si el usuario completó el campo
  ...
  /* ── Recontacto masivo por WhatsApp — abre el chat de cada cliente con el
     mensaje pre-cargado, de a uno (WhatsApp no permite envío masivo real). ── */
  ...
  const openBulkRecontact = () => {
    ...
    setRecontactMsg('Hola {nombre}! ¿Cómo venís? Hace un tiempo que no armamos un pedido y quería saber si estás necesitando reponer algo. Cualquier cosa quedo a disposición. ¡Saludos!')
  ...
  return (
    <div className="page active" style={{ animation: 'pgIn .2s ease both' }}>
                  ...
                  <tr key={c.id} onClick={() => openDetail(c)} style={{ background: selectedIds.has(c.id) ? 'rgba(124,58,237,.06)' : undefined }}>
                    ...
                    <td>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        ...
                        <div>
                          <div style={{ fontWeight: 700, fontSize: 13, color: 'var(--txt)', lineHeight: 1.3, fontFamily: "'Space Grotesk','Inter',sans-serif", letterSpacing: '-.2px' }}>{c.company || c.contact || '—'}</div>
                      ...
                      <div className="zt-acts">
                        {c.wa && (
                          <button
                            ...
                            style={{ width:28,height:28,borderRadius:'50%',border:'1.5px solid var(--border2)',background:isCold?'#FEF9C3':'var(--surface2)',color:isCold?'#EAB308':'var(--txt3)',cursor:'pointer',fontSize:10,display:'inline-flex',alignItems:'center',justifyContent:'center',padding:0,flexShrink:0,transition:'all .15s' }}>
                            <i className="fa fa-bolt" />
                          </button>
                        )}
                        <button title="Editar" onClick={() => openEdit(c)}
                          style={{ width:28,height:28,borderRadius:'50%',border:'1.5px solid var(--border2)',background:'var(--surface2)',color:'var(--txt2)',cursor:'pointer',fontSize:10,display:'inline-flex',alignItems:'center',justifyContent:'center',padding:0,flexShrink:0,transition:'all .15s' }}>
                          <i className="fa fa-pen" />
                        </button>
                        <button title="Eliminar" onClick={() => del(c.id)}
                          style={{ width:28,height:28,borderRadius:'50%',border:'1.5px solid #FECACA',background:'#FEF2F2',color:'#DC2626',cursor:'pointer',fontSize:10,display:'inline-flex',alignItems:'center',justifyContent:'center',padding:0,flexShrink:0,transition:'all .15s' }}>
                ...
                <div className="nc-qact" onClick={e => e.stopPropagation()}>
                  ...
                  {c.email && (
                    <a href={`mailto:${c.email}`} title={c.email} onClick={e => e.stopPropagation()}
                      style={{ width:30,height:30,borderRadius:'50%',border:'none',background:'#EFF6FF',color:'#2563EB',cursor:'pointer',fontSize:12,display:'inline-flex',alignItems:'center',justifyContent:'center',padding:0,flexShrink:0,textDecoration:'none' }}>
                      <i className="fa fa-envelope" />
                    </a>
                  )}
                  {c.wa && (
                    <a href={`tel:${c.wa.replace(/\D/g,'')}`} title={`Llamar · ${c.wa}`} onClick={e => e.stopPropagation()}
                      style={{ width:30,height:30,borderRadius:'50%',border:'none',background:'#EFF6FF',color:'#2563EB',cursor:'pointer',fontSize:12,display:'inline-flex',alignItems:'center',justifyContent:'center',padding:0,flexShrink:0,textDecoration:'none' }}>
        ...
        {loading ? Array.from({ length: 5 }, (_, i) => (
          ...
          return (
            <div key={c.id} className="cli-mob-card" onClick={() => openDetail(c)}
              ...
              {/* Checkbox de selección (para acciones masivas en móvil) */}
              ...
              <div style={{ flexShrink:0, position:'relative' }} onClick={e => e.stopPropagation()}>
                <button
                  ...
                  style={{ width:40,height:40,borderRadius:'50%',border:'1.5px solid var(--border2)',background:'var(--surface2)',color:'var(--txt2)',cursor:'pointer',fontSize:15,display:'inline-flex',alignItems:'center',justifyContent:'center',flexShrink:0,padding:0,WebkitTapHighlightColor:'transparent',touchAction:'manipulation' }}>
                ...
                {openMenuId === c.id && (
                  <div
                    ...
                    {c.wa && (
                      <button
                        onClick={e => { setOpenMenuId(null); openRevincul(c, e) }}
                        ...
                        <i className="fa fa-bolt" style={{ width:16, color: isCold ? '#EAB308' : 'var(--brand)' }} />
                        {isCold ? 'Recontactar' : 'Re-vincular WA'}
                      </button>
                    )}
                    <button
                    ...
                    <button
                      onClick={() => { setOpenMenuId(null); del(c.id) }}
      ...
      {/* ── Barra de acciones masivas (con etiquetas para que se entienda) ── */}
      {selectedIds.size > 0 && (
          ...
          <div className="modal-form-card" style={{ maxWidth: 560, minHeight:'auto', height:'auto', maxHeight:'90vh', overflow:'auto' }}>
            ...
            <div style={{ padding:'18px 24px 22px' }}>
              <p style={{ fontSize:12.5, color:'var(--txt2)', margin:'0 0 12px', lineHeight:1.55 }}>
                Editá el mensaje (poné <b>{'{nombre}'}</b> y se reemplaza por cada contacto). Tocá <b>Enviar</b> en cada cliente: se abre su chat con el mensaje listo para enviar.
              </p>
      ...
      {modal && (
        <div className="modal-bg open" onClick={e => { if (e.target === e.currentTarget) setModal(false) }}>
          ...
          <div className="modal-form-card" style={{ maxWidth: 680, minHeight: 'auto', height: 'auto', maxHeight: 'none', overflow: 'visible' }}>
            ...
            {/* Datos fiscales — COLAPSABLE. Auto-abierto si ya tiene datos cargados */}
            {(() => {
              ...
              return (
                <div style={{ borderTop: '1px solid var(--border)', margin: '14px 0 10px', paddingTop: 12 }}>
                  <button type="button" onClick={() => setShowFiscal(!isOpen)}
                    ...
                    <span style={{ fontSize: 10, fontWeight: 700, color: 'var(--txt3)', textTransform: 'uppercase', letterSpacing: '.08em', flex: 1 }}>
                      Datos Fiscales (Facturación)
                    </span>
                    <span style={{ fontSize: 10.5, color: 'var(--txt4)', fontWeight: 500 }}>
                      {isOpen ? 'Opcional' : 'Agregar si facturás'}
                    </span>
            ...
            </div>{/* /body — flujo natural */}
            {/* Footer — flujo natural, sin sticky */}
            <div className="mfooter" style={{ borderTop: '1px solid var(--border)', padding: '14px 28px 20px', display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
        ...
        <div className="modal-bg open" onClick={e => { if (e.target === e.currentTarget) { setImportModal(false); setCsvPreview([]) } }}>
          <div style={{ width: '100%', maxWidth: 620, background: 'var(--surface)', borderRadius: 16, display: 'flex', flexDirection: 'column', maxHeight: '92dvh', overflow: 'hidden', boxShadow: 'var(--sh-lg)', margin: 'auto', animation: 'pgIn .2s ease both' }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display: 'flex', borderBottom: '1px solid var(--border)', flexShrink: 0 }}>
              {[{ id: 'archivo', icon: 'fa-cloud-arrow-up', label: 'Subir archivo' }, { id: 'pegar', icon: 'fa-paste', label: 'Pegar números' }].map(t => (
              ...
              {csvPreview.length > 0 && (
                <div>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
                    ...
                    <button className="btn btn-ghost btn-xs" style={{ color: 'var(--txt3)' }} onClick={() => { setCsvPreview([]); if (fileRef.current) fileRef.current.value = '' }}><i className="fa fa-arrow-left" /> Cambiar</button>
                  </div>
                  <div style={{ overflowX: 'auto', border: '1px solid var(--border)', borderRadius: 10, maxHeight: 340, overflowY: 'auto' }}>
                    <table style={{ fontSize: 11, minWidth: 480 }}>
                      ...
                      <tbody>
                        {csvPreview.slice(0, 20).map((c, i) => (<tr key={i}><td style={{ color: 'var(--txt4)', textAlign: 'center' }}>{i+1}</td><td><b>{c.company || c.contact}</b></td><td>{c.contact}</td><td>{c.wa}</td><td>{c.email}</td><td>{c.rubro}</td></tr>))}
              ...
              {importTab === 'pegar' && (
                <div>
                  <div style={{ padding: '12px 14px', borderRadius: 10, background: '#FFF7ED', border: '1.5px solid #FED7AA', marginBottom: 14, fontSize: 12 }}>
                    <div style={{ fontWeight: 700, color: '#C2410C', marginBottom: 6, display: 'flex', alignItems: 'center', gap: 6 }}><i className="fa fa-bolt" /> Cargá todos tus clientes ahora mismo — sin exportar nada</div>
                    <div style={{ color: '#92400E', lineHeight: 1.6 }}>Abrí WhatsApp, entrá a cada chat de cliente y <b>copiá el número</b> desde el perfil del contacto. Pegalo acá (uno por línea) con o sin nombre. También podés pegar desde tus notas, agenda o cualquier lista.</div>
                  ...
                  <textarea value={pasteNums} onChange={e => setPasteNums(e.target.value)}
                    ...
                    style={{ width: '100%', minHeight: 180, borderRadius: 10, border: '1.5px solid var(--border)', padding: '12px 14px', fontSize: 13, fontFamily: 'inherit', lineHeight: 1.7, resize: 'vertical', outline: 'none', background: 'var(--surface)', color: 'var(--txt)', boxSizing: 'border-box' }}
                    autoFocus />
                    ...
                    <div style={{ marginTop: 12 }}>
                      ...
                      <div style={{ overflowX: 'auto', border: '1px solid var(--border)', borderRadius: 10, maxHeight: 240, overflowY: 'auto' }}>
                        <table style={{ fontSize: 11, minWidth: 340 }}>
                          ...
                          <tbody>
                            {parsedPaste.slice(0, 30).map((c, i) => (<tr key={i}><td style={{ color: 'var(--txt4)', textAlign: 'center' }}>{i + 1}</td><td><b>{c.company}</b></td><td style={{ color: c.wa ? 'var(--txt)' : 'var(--txt4)', fontStyle: c.wa ? 'normal' : 'italic' }}>{c.wa || 'sin número'}</td></tr>))}
                  ...
                  {pasteNums.trim() && parsedPaste.length === 0 && (
                    <div style={{ marginTop: 10, padding: '10px 14px', background: 'var(--surface2)', borderRadius: 8, fontSize: 12, color: 'var(--txt3)' }}><i className="fa fa-circle-info" style={{ marginRight: 6 }} />Escribí al menos un número o nombre para ver la vista previa.</div>
                  )}
                </div>
              )}
            </div>
            <div className="mfooter" style={{ flexShrink: 0, borderTop: '1px solid var(--border)', padding: '12px 20px' }}>
              ...
              {importTab === 'archivo' && (
                <button className="btn btn-primary" onClick={doImport} disabled={!csvPreview.length}><i className="fa fa-file-import" /> {csvPreview.length > 0 ? `Importar ${csvPreview.length} clientes` : 'Importar'}</button>
              )}
              {importTab === 'pegar' && (
          ...
          <div className="modal" style={{ maxWidth: 480 }} onClick={e => e.stopPropagation()}>
            ...
            {!revinculModal.wa && (
              <div style={{ fontSize: 11, color: '#DC2626', background: '#FEE2E2', borderRadius: 8, padding: '8px 12px', marginBottom: 8 }}>
                <i className="fa fa-triangle-exclamation" style={{ marginRight: 5 }} />Este cliente no tiene WhatsApp registrado.
              </div>
            )}
